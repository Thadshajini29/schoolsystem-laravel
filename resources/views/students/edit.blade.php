            ...
            <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="student_name" id="student_name" class="form-control @error('student_name') is-invalid @enderror" value="{{ old('student_name', $student->student_name) }}" required>
                        @error('student_name')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="grade_id" id="grade_id" class="form-select @error('grade_id') is-invalid @enderror" required>
                            ...
                            @foreach($grades as $grade)
                                <option value="{{ $grade->id }}" {{ old('grade_id', $student->grade_id) == $grade->id ? 'selected' : '' }}>{{ $grade->grade_name }}</option>
                            @endforeach
                        </select>
                        @error('grade_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="nic_num" id="nic_num" class="form-control @error('nic_num') is-invalid @enderror" value="{{ old('nic_num', $student->nic_num) }}">
                        @error('nic_num')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="gender" id="genderMale" value="Male" {{ old('gender', $student->gender) == 'Male' ? 'checked' : '' }}>
                            <label class="btn btn-outline-info" for="genderMale">
                                <i class="bi bi-gender-male me-2"></i>Male
                            </label>
                            
                            <input type="radio" class="btn-check" name="gender" id="genderFemale" value="Female" {{ old('gender', $student->gender) == 'Female' ? 'checked' : '' }}>
                            <label class="btn btn-outline-danger" for="genderFemale">
                                <i class="bi bi-gender-female me-2"></i>Female
                            </label>
                            
                            <input type="radio" class="btn-check" name="gender" id="genderOther" value="" {{ old('gender', $student->gender) == '' || old('gender', $student->gender) == null ? 'checked' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    @error('image')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <!-- New image preview will be inserted here by JavaScript -->
                </div>
